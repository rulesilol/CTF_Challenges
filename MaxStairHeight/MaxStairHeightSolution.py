...
def maxStairHeight(startHeight, endHeight, length):
    for estimateMaxHeight in range(0,200000):
        stepsTakenToMax = estimateMaxHeight - startHeight
        remainingSteps = length - stepsTakenToMax 
        estimateEndHeight = estimateMaxHeight - remainingSteps - 1

        if startHeight == 0 and endHeight == 0 and length == 0:
            return 0
        if estimateEndHeight == endHeight:
            return estimateMaxHeight
